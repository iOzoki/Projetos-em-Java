...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        String playerName = escaneiaInformação.nextLine();
        System.out.println("Certo, " + playerName + " é um prazer tê-lo nessa história.");
        ...
        if(opções == 1){
            System.out.println("Ao seguir o caminho iluminado, você encontra uma clareira tranquila, mas logo se depara com um rio furioso bloqueando o caminho. Você pode tentar construir uma ponte ou procurar por uma passagem secreta nas margens do rio.\nConstruir uma ponte(Opção 1): Leva você a enfrentar criaturas da floresta que não estão dispostas a deixá-lo prosseguir facilmente.\nProcurar uma passagem secreta(Opção 2): Você descobre uma caverna oculta, mas está cheia de enigmas e perigos que testarão sua inteligência e habilidades.");
            opções = escaneiaInformação.nextInt();
            if(opções == 1){
                System.out.println("Construir uma ponte: Depois de enfrentar as criaturas da floresta e construir uma ponte sobre o rio, você finalmente alcança o outro lado. No entanto, ao atravessar, você descobre que a ponte está instável e desaba, deixando-o preso em uma parte desconhecida da floresta, sem uma maneira fácil de voltar.");
            }
            else if(opções == 2){
